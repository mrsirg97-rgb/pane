...
- `memory_fts.rowid` = `memories.id` — the FTS row for memory `m42` lives at rowid 42. Insert copies the content at learn time (gated on the per-open FTS capability); prune removes `DELETE FROM memory_fts WHERE rowid = <id>`. A test asserts no orphaned FTS row survives a prune.
...
Selection for remove/reduce is `ids` OR criteria (`scope`/`kind`/`older_than_days`). Cross-field requirements are runtime-loud, exactly the todo pattern: schema-optional fields, loud execute-time checks.
