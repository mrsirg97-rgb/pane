...
test("query is encoded and sent to the local SearXNG JSON API", async () => {
  ...
  assert.match(
    seen.url,
    /^http:\/\/127\.0\.0\.1:8888\/search\?q=rust%20simd%20%26%20memchr&format=json$/,
  );
...
test("maxResults slices, default is 5", async () => {
  ...
  try {
    ...
    const two = JSON.parse(
      textOf(await tool.execute("t4", { query: "q", maxResults: 2 })),
    );
...
test("missing fields degrade to empty strings, empty results say so", async () => {
  ...
  try {
    const parsed = JSON.parse(
      textOf(await tool.execute("t5", { query: "q", maxResults: 1 })),
    );
  ...
  try {
    assert.equal(
      textOf(await tool.execute("t6", { query: "q" })),
      "no results",
    );
  } finally {
    restoreEmpty();
...
test("SearXNG being down surfaces as a loud error", async () => {
  ...
  try {
    await assert.rejects(
      tool.execute("t7", { query: "q" }),
      /SearXNG search failed: HTTP 502/,
    );
  } finally {
    restore();
  }
...
test("schema requires query and bounds maxResults", async () => {
  ...
  assert.equal(
    Value.Check(tool.parameters, { query: "x", maxResults: 21 }),
    false,
  );
  assert.equal(
    Value.Check(tool.parameters, { query: "x", maxResults: 1 }),
    true,
  );
});
