...
const { handlers, exports: ext } = await loadExtensionTool(
...
test("row one drops branch when narrow, hides thinking for non-reasoning models", () => {
  const [narrow] = buildFooterLines(
    state({ branch: "a-very-long-branch-name-that-cannot-fit-here" }),
    theme,
    30,
  );
  ...
  const usage = collectUsage([
    {
      ...
      message: {
        role: "assistant",
        usage: { input: 100, output: 50, cacheRead: 300, cacheWrite: 20 },
      },
    },
    {
      type: "message",
    ...
    {
      type: "compaction",
      usage: { input: 40, output: 30, cacheRead: 0, cacheWrite: 0 },
    },
    {
      type: "message",
      message: {
        role: "assistant",
        usage: { input: 50, output: 25, cacheRead: 450, cacheWrite: 0 },
      },
    },
    { type: "session" },
  ]);
