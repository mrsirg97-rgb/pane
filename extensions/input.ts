...
import type { GlyphState } from "./types/input.types.ts";
...
export class FlatEditor extends CustomEditor {
  ...
  constructor(
    ...
    super(
      tui,
      { ...editorTheme, borderColor: (s: string) => paint("borderMuted", s) },
      keybindings,
      { paddingX: 2 },
    );
  ...
  render(width: number): string[] {
    ...
    if (first >= 0) {
      const glyph = this.glyphState.streaming
        ? this.paint("dim", "◐")
        : this.paint(this.focused ? "accent" : "dim", "❯");
...
export default function inputExtension(pi: ExtensionAPI) {
  ...
  pi.on("session_start", (_event, ctx) => {
    ...
    const paint = (color: string, text: string) =>
      (ctx.ui.theme as any).fg(color, text);
    ctx.ui.setEditorComponent(
      (tui, editorTheme, keybindings) =>
        new FlatEditor(tui, editorTheme, keybindings, glyphState, paint),
    );
  });
}
